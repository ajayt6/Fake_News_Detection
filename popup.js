...
function getUncommon(sentence, common) {
    ...
    //create list of negation words
    negation = negation.split(',');
    for ( i = 0; i < negation.length; i++ ) {
        negationObj[ negation[i].trim() ] = true;
    }
		...
		xmlHttp.onreadystatechange = function(theUrl) {
			...
			{	//alert(xmlHttp.responseText);
				...
				if (flag == 0) {
					...
					{
						//Change html body accordingly
						document.body.innerHTML = "<img style='width:210px;height:210px' src='http://www.snopes.com/app/themes/snopes-theme/dist/images/det-green.gif' >"; //"False";
					}
					else if ( xmlHttp.response.includes("http://www.snopes.com/app/themes/snopes-theme/dist/images/det-green.gif") )
					{
						document.body.innerHTML = "<img height='210' width='210' src='http://www.snopes.com/app/themes/snopes-theme/dist/images/det-red.gif' >"; //"True";
					}
					else
					{
						document.body.innerHTML = "<img height='210' width='210' src='http://worldartsme.com/images/unsure-clipart-1.jpg' >"//"Sorry. Truth check does not have sufficient info. You decide!";
